...
pbmc_part_metastatic_R<-readRDS("20210228_14sample_fil_Bcell_pbmc_reclusteringpbmc_pure_1_half.rds")
...
pdf(file=paste0("Figure3B_",Sys.Date(),"B_cell_DimPlot_dbscan.pdf"))
print(DimPlot(pbmc, reduction = "umap",cols=palette_seurat_clusters,pt.size=0.3))
dev.off()
pdf(file=paste0("Figure3C_",Sys.Date(),"B_cell_DimPlot_dbscan_puro.pdf"))
...
dev.off()
pdf(file=paste0("Figure3E_",Sys.Date(),"B_cell_DimPlot_meta.pdf"),6,5)
...
pdf(file=paste0("Figure3D_",Sys.Date(),"B_cellS6_DoHeatmap_dbscan.pdf"),10,15)
...
pdf(file=paste0("Figure3F_",Sys.Date(),"B_cells_geom_bar_dbscaLR_mate.pdf"))
gender_plot
dev.off()

#################plot Hdbscan clusters percentage figure S3 B
...
pdf(file=paste0("FigureS3B_",Sys.Date(),"B_cells_geom_bar_dbscaLR_MNn.pdf"))
gender_plot
dev.off()


pdf(file=paste0("Figure3g_",Sys.Date(),"B_cell_DimPlot_dbscan_puro.pdf"))
palette_seurat_clusters <- randomColor(length(unique(pbmc_pure$seurat_clusters)))
print(DimPlot(pbmc_pure_1, reduction = "umap",cols=palette_seurat_clusters,pt.size=0.3))
dev.off()

pdf(file=paste0("FigureS3C_",Sys.Date(),"B_cell_culs1_DimPlot_LR_MN.pdf"),7,5)
...
pdf(file=paste0("Figure3H_",Sys.Date(),"B_cells_geom_bar_kmeans_LR_MN.pdf"))
gender_plot
dev.off()
...
pdf(file=paste0("Figure3I_",Sys.Date(),"B_cell_plot_cell_trajectory_dbscan_puro1_meta.pdf"))
print(plot_cell_trajectory(HSMM_myo, color_by= "meta")+ scale_color_manual(breaks = waiver(),values=palette_meta))
dev.off()


plot_gene <- "IGHG4"
pdf(file=paste0("Figure3J_",Sys.Date(),"B_cell_plot_cell_trajectory_dbscan_puro1_",plot_gene,".pdf"))
