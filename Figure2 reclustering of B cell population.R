...
################################################################
#########Figure 2A clustering of all cell population###########
################################################################
#load files
X21_P.pbmc.data <- Read10X(data.dir = paste0("outs/filtered_feature_bc_matrix"))
...
pdf(paste0("Figure2A_",Sys.Date(),"all_cells_list_umap_meta_sample.pdf"))
plot(all_cells_list_tsne$UMAP_1,all_cells_list_tsne$UMAP_2,col=as.factor(all_cells_list_tsne$meta), cex = 0.1)
dev.off()

################################################################
#########Figure 2B re-clustering of B cell population###########
################################################################
...
pdf(file=paste0("Figure2B_",Sys.Date(),"20200731_T_cellreplot_tsne_4_groups.pdf"),10,8)
...
################################################################
#########Figure 2F Pseudotime analysis metastatic-L#############
################################################################
...
################################################################
#########Figure 2G Pseudotime analysis metastatic-R#############
################################################################
...
################################################################
####Figure 2C and 2D re-clustering of Activated B cell##########
################################################################
...
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_meta_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,9)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "umap", group.by = "meta",cols =c("#7BC27A","#336087"),pt.size=1, repel = T ))
dev.off()
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_side_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,9)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "umap", group.by = "side",cols =c("#7BC27A","#336087"),pt.size=1, repel = T))
dev.off()
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_T_MATA_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,9)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "tsne", group.by = "meta",cols =c("#7BC27A","#336087"),pt.size=1, repel = T ))
dev.off()
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_T_side_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,9)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "tsne", group.by = "side",cols =c("#7BC27A","#336087"),pt.size=1, repel = T))
dev.off()
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_tsne_LR_MN_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,7)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "tsne", group.by = "LR_MN",pt.size=1, repel = T))
dev.off()
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_LR_MN_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,7)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "umap", group.by = "LR_MN",pt.size=1, repel = T))
dev.off()
pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_tsne_cluseter_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,10)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "tsne",pt.size=1, repel = T))
dev.off()

pdf(file=paste0("Figure2C",patient_name,LSEC_population,Sys.Date(),"MT_cluseter_20200731_B_cellreplot_tsne_active_Bcell_groups.pdf"),10,10)
print(DimPlot(pbmc_part_actived_B_Cell, reduction = "umap",pt.size=1, repel = T))
dev.off()


pdf(paste0("FigureD_FeaturePlot",Sys.Date(),".pdf"),15,10)
...
pdf(paste0("Figure2D_acvite_BC_hsBCL9_single_cell_cacer_atlas_cancertype_Seurat_DoHeatmap10_",Sys.Date(),".pdf"),4,5)
...
################################################################
##########Figure 2C Number of cells proportion bar plot#########
################################################################
...
pdf(paste0("Figure 2C_bar_plot_",Sys.Date(),".pdf"),15,10)
barpt
dev.off()
